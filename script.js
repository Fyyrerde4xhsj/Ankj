/* ============================
   🧠 BRAINFORGE ENGINE v1.1
   With Unity Ads Integration
   ============================ */

/**
 * UNITY ADS INITIALIZATION
 */
// ⚠️ REPLACE '1234567' WITH YOUR ACTUAL GAME ID FROM UNITY DASHBOARD
let gameId = '1234567'; 
let rewardedPlacementId = 'rewardedVideo'; // Default placement ID

// Define the Ad Listener
let adListener = {
    onUnityAdsReady: function(placementId) {
        console.log('Ad ready: ' + placementId);
    },
    onUnityAdsStart: function(placementId) {
        console.log('Ad started');
        // Optional: Pause background music here if you have any
    },
    onUnityAdsFinish: function(placementId, state) {
        if (state === 'COMPLETED') {
            console.log('User watched full ad.');
            
            // REWARD LOGIC: Give user extra 50 XP and boost streak
            AppStore.data.xp += 50;
...
// Initialize SDK safely (check if script loaded)
if (typeof unityAds !== 'undefined') {
    unityAds.setReadyListener(adListener.onUnityAdsReady);
    unityAds.init(gameId, adListener);
} else {
    console.warn("Unity Ads SDK not loaded. Check internet connection or AdBlock.");
}
...
const GameEngine = {
    ...
    endGame() {
        ...
        // ------------------------------------------
        // TRIGGER UNITY AD ON GAME FINISH
        // ------------------------------------------
        if (typeof unityAds !== 'undefined' && unityAds.getPlacementState(rewardedPlacementId) === 'READY') {
            console.log("Triggering Unity Ad...");
            unityAds.show(rewardedPlacementId);
        } else {
            console.log('Ad not ready or SDK not loaded, skipping.');
        }
    }
};

/**
 * GAME MODULES (Content Generation)
 */
const Games = {
    // 1. MATH RUSH
    math: {
